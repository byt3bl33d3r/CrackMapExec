# Copyright (c) 2003-2015 CORE Security Technologgingies
#
...
class ServiceInstall:
    ...
    def getShares(self):
        ...
        logging.info("Requesting shares on %s....." % (self.connection.getRemoteHost()))
        try: 
            ...
            logging.critical("Error requesting shares on %s, aborting....." % (self.connection.getRemoteHost()))
            raise

        
    def createService(self, handle, share, path):
        logging.info("Creating service %s on %s....." % (self.__service_name, self.connection.getRemoteHost()))
        ...
        try: 
            ...
            logging.critical("Error creating service %s on %s" % (self.__service_name, self.connection.getRemoteHost()))
            raise
        else:
            return resp['lpServiceHandle']

    def openSvcManager(self):
        logging.info("Opening SVCManager on %s....." % self.connection.getRemoteHost())
        # Setup up a DCE SMBTransport with the connection already in place
        ...
        try:
            ...
            logging.critical("Error opening SVCManager on %s....." % self.connection.getRemoteHost())
            raise Exception('Unable to open SVCManager')
        else:
            return resp['lpScHandle']

    def copy_file(self, src, tree, dst):
        logging.info("Uploading file %s" % dst)
    ...
    def install(self):
        if self.connection.isGuestSession():
            logging.critical("Authenticated as Guest. Aborting")
            self.connection.loggingoff()
            del self.connection
        else:
            fileCopied = False
            ...
            try:
                ...
                if svcManager != 0:
                    ...
                    if service != 0:
                        # Start service
                        logging.info('Starting service %s.....' % self.__service_name)
                        try:
                ...
                logging.critical("Error performing the installation, cleaning up: %s" %e)
                try:
    ...
    def uninstall(self):
        ...
        try:
            ...
            if svcManager != 0:
                ...
                logging.info('Stoping service %s.....' % self.__service_name)
                try:
                    scmr.hRControlService(self.rpcsvc, service, scmr.SERVICE_CONTROL_STOP)
                except:
                    pass
                ...
                scmr.hRCloseServiceHandle(self.rpcsvc, service)
                scmr.hRCloseServiceHandle(self.rpcsvc, svcManager)
            logging.info('Removing file %s.....' % self.__binary_service_name)
            self.connection.deleteFile(self.share, self.__binary_service_name)
        except Exception:
            logging.critical("Error performing the uninstallation, cleaning up" )
            try:
                scmr.hRControlService(self.rpcsvc, service, scmr.SERVICE_CONTROL_STOP)
            except:
                pass
