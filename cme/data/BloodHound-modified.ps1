    ...
    END {
        ...
        if ($PSCmdlet.ParameterSetName -eq 'CSVExport') {
            ...
            if($SessionWriter) {
                ...
                Get-Content "$OutputFolder\$($CSVExportPrefix)user_sessions.csv" 
                Write-Output "!-!"
                Remove-Item "$OutputFolder\$($CSVExportPrefix)user_sessions.csv" 
            }
            if($GroupWriter) {
                $GroupWriter.Dispose()
                $GroupFileStream.Dispose()
                Get-Content "$OutputFolder\$($CSVExportPrefix)group_memberships.csv" 
                Write-Output "!-!"
                Remove-Item "$OutputFolder\$($CSVExportPrefix)group_memberships.csv" 
            }
            if($ACLWriter) {
                $ACLWriter.Dispose()
                $ACLFileStream.Dispose()
                Get-Content "$OutputFolder\$($CSVExportPrefix)acls.csv" 
                Write-Output "!-!"
                Remove-Item "$OutputFolder\$($CSVExportPrefix)acls.csv" 
            }
            if($LocalAdminWriter) {
                $LocalAdminWriter.Dispose()
                $LocalAdminFileStream.Dispose()
                Get-Content "$OutputFolder\$($CSVExportPrefix)local_admins.csv" 
                Write-Output "!-!"
                Remove-Item "$OutputFolder\$($CSVExportPrefix)local_admins.csv" 
            }
            if($TrustWriter) {
                $TrustWriter.Dispose()
                $TrustsFileStream.Dispose()
                Get-Content "$OutputFolder\$($CSVExportPrefix)trusts.csv" 
                Write-Output "!-!"
                Remove-Item "$OutputFolder\$($CSVExportPrefix)trusts.csv"  
            }
