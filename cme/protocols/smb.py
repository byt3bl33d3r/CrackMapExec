...
class smb(connection):
    ...
    def proto_args(parser, std_parser, module_parser):
        ...
        egroup.add_argument('--dcs', action='store_true', help='enumerate domain controllers')
        ...
        egroup.add_argument("--rid-brute", nargs='?', type=int, const=4000, metavar='MAX_RID', help='enumerate users by bruteforcing RID\'s')
        egroup.add_argument("--wmi", metavar='QUERY', type=str, help='issues the specified WMI query')
        egroup.add_argument("--wmi-namespace", metavar='NAMESPACE', default='root\\cimv2', help='WMI Namespace')
        ...
        sgroup.add_argument("--spider-folder", metavar='FOLDER', default='.', type=str, help='folder to spider')
        ...
        sgroup.add_argument("--depth", type=int, default=None, help='max spider recursion depth')
        ...
        cegroup = cgroup.add_mutually_exclusive_group()
        cegroup.add_argument("-x", metavar="PS_COMMAND", dest='execute', help='execute the specified PowerShell command')
    ...
    def dcs(self):
        ...
        for dc_ip in self.get_dc_ips():
            try:
                dcs = get_netdomaincontroller(dc_ip, self.domain, self.username, self.password, self.lmhash, self.nthash)  # queried_domain=str())
                for dc in dcs:
                    hostname = dc.dnshostname
                    name = dc.cn
                    domain = dc.dnshostname.split('.')[1].upper()
                    os = dc.operatingsystem
                    self.logger.highlight('Hostname: {} Name: {} Domain: {} OS: {}'.format(hostname, name, domain, os))
            ...
            try:
                ...
                for user in users:
                    ...
                    self.logger.highlight(
                        '{}\\{:<30} badpwdcount: {} baddpwdtime: {}'.format(
                            domain, user.samaccountname, getattr(user, 'badpwdcount', 0),
                            getattr(user, 'badpasswordtime', '')
                        )
                    )
    ...
    def ntds(self):
        ...
        if self.remote_ops and self.bootkey:
            try:
                ...
                NTDS = NTDSHashes(
                    NTDSFileName, self.bootkey, isRemote=True, history=False, noLMHash=True,
