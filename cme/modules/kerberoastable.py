...
class CMEModule:
    ...
    '''
    name = 'kerberoastable'
    description = 'Enumerate Kerberoastable users'
    ...
    def on_login(self, context, connection):
        ...
        search_filter = '(&(objectCategory=person)(objectClass=user)(servicePrincipalName=*)(userAccountControl:1.2.840.113556.1.4.803:=512))'
        ...
        if len(users) > 0:
            context.log.success('Found the following Kerberoastable users:')
            for user in users:
                context.log.highlight('User: {} '.format(user[0]))
        else:
            context.log.info('No Kerberoastable users found')
